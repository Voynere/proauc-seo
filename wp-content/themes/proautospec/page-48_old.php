...
<?php 
	if (($post->api_meta->isVendor) || ($post->api_meta->isModel)) {
		...
		$vendor = $wpdb->get_row('SELECT * FROM wp_api_vendors WHERE country = "'.$country.'" AND UPPER(vendor_label) = "'.$post->api_meta->vendorLabel.'" AND seo_title IS NOT NULL group by vendor_label');
		?>
