...
Promise.all([domLoaded]).then((results) => {
	...
	if(urlParams.has('marka-name')){
		apiParams.append( 'marka_name', urlParams.get('marka-name'));
	}		
	if(urlParams.has('model-name')){
		apiParams.append( 'model_name', urlParams.get('model-name'));
	}	
	...
	let page_size = 16, k_page_select = 1;
	if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|BB|PlayBook|IEMobile|Windows Phone|Kindle|Silk|Opera Mini/i.test(navigator.userAgent)) {
		var max_pagination = 2;
		var max_pagination_size = 5;
	}else{
		var max_pagination = 6;
		var max_pagination_size = 11;
	}
	...
	function loadData(params, pageNumber){
		...
		console.log('api request: ' + '/api/get-motorcycles.php?'+params);
		jQuery.ajax({
			url: API_URL + '/api/get-motorcycles.php?'+params,
			method: 'get',
			dataType: 'json',
			success: function(data){
				...
				jQuery('.cars-listing__total').html( 'Всего по запросу найдено <var>' + data.count +'</var> мотоциклов: ');
			...
			error: function(){
				...
				jQuery('.k_auto_content').append('<div class="cart_new cart_not-found"><p class="cart_not-found_title">Не найдено</p></div>');
			}
		});
	...
	function renderListing(data){
		...
		for (var i = 0; i < data.length; i++) {
			...
			let itemDetailedUrl = '/motorcycles/'+ car.id + '-' + nameInUrl + '/';
	


			
			if (car.model_name.includes( car.marka_name )){
				itemHtml.find('.car-model-name').html(car.model_name);
			}else {
			...
			*/
			...
			if(images.length > 1){
				...
				for (let j=0; j < ( 5 < images.length ? 5 : images.length); j++){
					...
					$curItem.attr({'src': images[j].replace('http://', 'https://').replace('&h=50', '')});
