...
function getMarkaBySlug(slug) {
	return models.filter(
		function(data){ 
			
			return data.text.toLowerCase().replace(/\s+/g, '-') == slug }
	);
}

function getMarkaById(id) {
	return models.filter(
		function(data){ return data.id == id }
	);
}

function getModelIdBySlug(slug, markModels){
	return markModels.filter(
		function(data){ return data.text.toLowerCase().replace(/\s+/g, '-') == slug }
	);
}
...
Promise.all([domLoaded]).then((results) => {
	...
	if (window.location.href.includes('iz-korei')){
		models = JSON.parse(models_korea);
		baseUrl = '/avto-iz-korei/catalog/';
		API_URL = '/api/get-cars-korea.php';
		routerParams.country = 'korea';
		routerParams.countryLabel = 'из Кореи';
	}else if (window.location.href.includes('iz-yaponii')){
		models = JSON.parse(models_japan);
		baseUrl = '/avto-iz-yaponii/catalog/';
		API_URL = '/api/get-cars-japan.php';
		routerParams.country = 'japan';
		routerParams.countryLabel = 'из Японии';
		if (window.location.href.includes('statistika')){
			models = JSON.parse(models_japan);
			baseUrl = '/avto-iz-yaponii/statistika/';
			API_URL = '/api/get-cars-japan.php';
			routerParams.country = 'japan';
			routerParams.countryLabel = 'из Японии';
		}
	}else if (window.location.href.includes('iz-kitaya')){
		models = JSON.parse(models_china);
		baseUrl = '/avto-iz-kitaya/catalog/';
		API_URL = '/api/get-cars-china.php';
		routerParams.country = 'china';
		routerParams.countryLabel = 'из Китая';
	}else if (window.location.href.includes('motorcycles')){
		models = JSON.parse(models_bike);
		baseUrl = '/motorcycles/';
		API_URL = '/api/get-motorcycles.php';
	}else if (window.location.href.includes('gruzoviki')){
		models = JSON.parse(models_trucks);
		baseUrl = '/spectehnika/gruzoviki/';
		API_URL = '/api/get-trucks.php';
	}
	//console.log(models);

	
	router.add (baseUrl + ':mark/', function () {});
	router.add (baseUrl + ':mark/:model/', function () {});	
	router.run(url.pathname);

	if (routerParams.mark != null){
		urlParams.delete('mark-id');
		let mark;
		try {
			
			mark = getMarkaBySlug(routerParams.mark);
			//console.log(mark);
			urlParams.set('mark-id', mark[0].id);
		} catch (error) { 
			//console.log(error);
			urlParams.delete('mark-id');
			window.location.replace(baseUrl);
		}
		if (routerParams.model != null){
			try {
				let modelId = getModelIdBySlug(routerParams.model, mark[0].models);
				//console.log('model id from url:');
				//console.log(modelId[0].id);
				urlParams.delete('model-id');
				urlParams.set('model-id', modelId[0].id);
			} catch (error) { 
				//console.log(error);
				urlParams.delete('model-id');
				window.location.replace(baseUrl + routerParams.mark + '/');
	...
	jQuery("#mark-id").empty().select2({  placeholder: '', data:[{'id': '', 'text': 'Выберите марку'}] });
	jQuery('#mark-id').select2({data: Object.values(models)});
	jQuery("#model-id").empty().select2({   placeholder:'Выберите модель', data:[] });


	jQuery('body').on('change', '#mark-id', function(){
		if (jQuery(this).val() !=""){
			let val = jQuery(this).val();
			jQuery("#model-id").empty().select2({ data:[{'id': '', 'text': 'Выберите модель'}] });
			jQuery("#model-id").select2({ data: Object.values(getMarkaById(val)[0].models) });
		}else{
			jQuery("#model-id").empty().select2({   placeholder:'Выберите модель', data:[{
					'id': '', 'text': 'Выберите модель',selected:'selected'
				}] });
		}
	...
	jQuery('#year-start').empty().select2({
	  tags: true,	  
	  data: yearsReverse,
	  placeholder: '2000',
	...
	jQuery('#mileage-start').empty().select2({
	  tags: true,	  
	  data: mileage,
	  placeholder: '100',
	  escapeMarkup: function(markup) {return markup; }
	...
	jQuery('#mileage-end').empty().select2({
	  tags: true,	  
	  data: mileageReverse,
	  placeholder: '500 000',
	  escapeMarkup: function(markup) {return markup; }
	...
	jQuery('.b-cars-catalog-filter--goto form').on('submit', function(e){
		...
		if (jQuery('#mark-id').val() && typeof(jQuery('#mark-id').val()) !== "undefined") {
			...
			var markSlug = jQuery('#mark-id').select2('data')[0]['text'].toLowerCase().replace(/\s+/g, '-');
			jQuery('#mark-id').attr('disabled', 'disabled');
			submitUrl = submitUrl + markSlug + '/';
		
		
			if(jQuery('#model-id').val() && typeof(jQuery('#model-id').val()) !== "undefined"){
				//filterParams.append ('mark-id', jQuery('#mark-id').val());
				var modelSlug = jQuery('#model-id').select2('data')[0]['text'].toLowerCase().replace(/\s+/g, '-');
				jQuery('#model-id').attr('disabled', 'disabled');
				submitUrl = submitUrl + modelSlug + '/';	
			}
		}
